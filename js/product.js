(function () {
	...
	function goTo(idx) {
		...
		setTimeout(function () { going = false; }, 700);
	}
	...
	document.querySelectorAll('.gnb-dropdown a').forEach(function (link) {
		link.addEventListener('click', function (e) {
			...
			if (!pagePart || pagePart === curPage) {
				e.preventDefault();
				var idx = -1;
				sections.forEach(function (s, i) { if ('#' + s.id === hash) idx = i; });
				if (idx >= 0) goTo(idx);
			}
		});
	});
	...
	window.addEventListener('hashchange', function () {
		...
		var idx = -1;
		sections.forEach(function (s, i) { if ('#' + s.id === location.hash) idx = i; });
		if (idx >= 0) goTo(idx);
	});
	...
	window.__prodGoToNext = function () { goTo(activeIdx + 1); };
	window.__prodGoToPrev = function () { goTo(activeIdx - 1); };

	/* 초기 탭 결정 */
	var initIdx = 0;
	if (location.hash) {
		sections.forEach(function (s, i) { if ('#' + s.id === location.hash) initIdx = i; });
	}
	sections.forEach(function (s, i) { s.style.display = i === initIdx ? '' : 'none'; });
	activeIdx = initIdx;
