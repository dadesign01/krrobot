...
(function () {
	...
	function place(withTransition) {
		const tr = withTransition
			? 'transform 1s cubic-bezier(0.25, 0.46, 0.45, 0.94), width 1s cubic-bezier(0.25, 0.46, 0.45, 0.94), opacity 0.7s ease'
			: 'none';
		const vw     = window.innerWidth;
		...
		allSlides.forEach((s, i) => {
			...
			if (offset === 0) {
				...
				// 화면 밖
				s.style.transform = offset < 0
					? `translateX(${-(activeW + 60)}px)`
					: `translateX(${vw + 60}px)`;
	...
	function prev() { move(trackPos - 1, ((current - 1) + total) % total); }
	function next() { move(trackPos + 1,  (current + 1) % total); }
	function goTo(i) { move(i + 1, i); }
	...
	window.addEventListener('resize', () => { calcSizes(); place(false); });
})();
